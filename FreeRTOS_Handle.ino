...
void ARMYA_Mot(void* pvParameters) {
  ...
  while (1) {
    ...
    if (abs(ArmYA[0] - ArmYA[1]) > 1) {  // Initate motor function
      ...
      for (int St1 = 0; St1 <= SHY_steps; St1++) {
    ...
    //   /* Include function to convert angles to step, 400 steps = 360 */
    //   uint32_t SHY_steps = (round(abs(SHY_Error) / 0.9) ) * 16; /* Inside 16:1 Gearbox */
...
void ARMPA_Mot(void* pvParameters) {
  ...
  while (1) {
    if (ArmPA[0] != ArmPA[1]) {  // Shoulder Pitch
      xSemaphoreTake(motorSemaphore, portMAX_DELAY);
      Serial.println("Shoulder Pitch Change!");                                     // Flag enable
      int ArmPA_D = abs(ArmPA[0] - ArmPA[1]) * (200000);  //P Delay
      if (ArmPA[0] < ArmPA[1]) {
        for (; ArmPA[0] >= ArmPA[1]; ArmPA[0]++) {
          SHP.write(ArmPA[0] + ARMPA_Offset);
          delayMicroseconds(ArmPA_D);
        }
      }
...
void ARMRA_Mot(void* pvParameters) {
  ...
  while (1) {
    if (ArmRA[0] != ArmRA[1]) {  // Shoulder Roll
      xSemaphoreTake(motorSemaphore, portMAX_DELAY);
      Serial.println("Shoulder Roll Change!");                                // Flag enable
      int ArmRA_D = abs(ArmRA[0] - ArmRA[1]) * (200000);  //P Delay
      if (ArmRA[0] < ArmRA[1]) {
        for (; ArmRA[0] >= ArmRA[1]; ArmRA[0]++) {
          SHR.write(ArmRA[0] + ARMRA_Offset);
          delayMicroseconds(ArmRA_D);
        }
      }
...
void ELPA_Mot(void* pvParameters) {
  ...
  while (1) {
    if (ElbowPA[0] != ElbowPA[1]) {  // Elbow Pitch
      xSemaphoreTake(motorSemaphore, portMAX_DELAY);
      Serial.println("Elbow Pitch Change!");                      // Flag enable
      int ElbowPA_D = abs(ElbowPA[0] - ElbowPA[1]) * (200000);  //P Delay
      if (ElbowPA[0] < ElbowPA[1]) {
        for (; ElbowPA[0] >= ElbowPA[1]; ElbowPA[0]++) {
          EP.write(ElbowPA[0] + ELPA_Offset);
          delayMicroseconds(ElbowPA_D);
        }
      }
...
void WRPA_Mot(void* pvParameters) {
  ...
  while (1) {
    if (WristPA[0] != WristPA[1]) {  // Wrist Pitch
      ...
      int WristPA_D = abs(WristPA[0] - WristPA[1]) * (20000);  //P Delay
      if (WristPA[0] < WristPA[1]) {
        for (; WristPA[0] >= WristPA[1]; WristPA[0]++) {
          FP.write(WristPA[0]);
          delayMicroseconds(WristPA_D);
        }
      }
...
void Sensor_Feedback(void* pvParameters) {
  ...
  while (1) {
    ...
    UpperArmSensor(); /* Read and Declare Angle */
  }

  if (HandTrack == true && ObjFound == true) {
    /* Tracking Object */
    Hand_InvsKin(objX, objY, WristRA[1], WristPA[1], &WristRA[0], &WristPA[1]);
  }
}
